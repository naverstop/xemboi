...
def _adjust_time(birth: BirthInput, solar_d: date) -> tuple[date, time | None]:
    """시각 보정. 시간 없으면 그대로.
    ① 서머타임 자동 제거(항상): 기록 시각(현지 시계) → 그 시기 표준시.
       한국 서머타임 1948~51·55~60·87~88 출생자는 시계가 1시간 빨라 시(時)가 어긋남.
    ② 진태양시(옵션): 표준시 → 현지 태양시 = 경도 보정(+ 균시차).
       => 진태양시 ON은 결과적으로 현지 평(진)태양시 = 시각 − UTCoffset + 경도/15h."""
    ...
    total_min = -(off_h - civil_h) * 60.0      # ① 서머타임 제거(표준시 환산)

    if birth.apply_true_solar_time:             # ② 경도(그 시기 표준자오선=civil_h×15°) 보정
        lon = birth.birth_longitude if birth.birth_longitude is not None else default_lon
        total_min += (lon - civil_h * 15.0) * 4.0
        if birth.apply_equation_of_time:
            total_min += _equation_of_time_min(solar_d)

    if total_min:
        dt += timedelta(minutes=round(total_min))
...
def compute_pillars(birth: BirthInput) -> tuple[FourPillars, date, time | None, date, bool]:
    ...
    adj_d, adj_t = _adjust_time(birth, solar_d)

    # 자시(子時, 23:00~24:00) 관법 처리.
    ...
    is_late_zi = adj_t is not None and adj_t.hour == 23
    if is_late_zi and birth.night_zi_mode == "jeongja":
        eff = adj_d + timedelta(days=1)               # 23시 → 익일로 굴림
        ...
        day = sxtwl.fromSolar(adj_d.year, adj_d.month, adj_d.day)
        ...
        if adj_t is not None:
            # sxtwl.getHourGZ(23) 은 자시 천간을 익일 일간 기준으로 계산(야자시법).
            hour_p = _gz_to_pillar(day.getHourGZ(adj_t.hour))

    if birth.locale == "vi":
        # 베트남 음력 표시는 호응옥득(UTC+7)로 — day 가 대표하는 양력일 기준(자시 굴림 반영).
        from .hongoc_duc import solar_to_lunar
        _ld, _lm, _ly, _leap = solar_to_lunar(
            day.getSolarDay(), day.getSolarMonth(), day.getSolarYear(), 7.0
        )
        lunar_d = date(_ly, _lm, _ld)
        is_leap = _leap
    else:
        lunar_d = date(day.getLunarYear(), day.getLunarMonth(), day.getLunarDay())
