...
# 확장 해설(전문가 시드 기반 anti-padding 확장): code -> {enriched_up, enriched_rev, src}.
# interp_for 가 이 계층을 최우선 반환(없으면 전문가 오버레이/시드로 폴백). data/tarot_enriched_kr.json.
_ENRICHED_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "tarot_enriched_kr.json"
)
_enriched: dict[str, dict[str, Any]] = {}
...
def _ensure_loaded() -> None:
    ...
    with _lock:
        ...
        _cards = cards
        # 확장 해설 로드(best-effort — 파일 없으면 전문가 오버레이/시드로 폴백, 서비스 영향 0)
        try:
            if os.path.exists(_ENRICHED_PATH):
                with open(_ENRICHED_PATH, encoding="utf-8") as ef:
                    _enriched.clear()
                    _enriched.update(json.load(ef) or {})
        except Exception:  # noqa: BLE001 — 확장은 선택적 보강, 실패해도 시드로 정상 동작
            _enriched.clear()
...
def interp_for(code: str, is_reversed: bool) -> str:
    ...
    """
    _ensure_loaded()
    # 확장 해설 최우선(전문가 시드 기반 anti-padding 확장) → 전문가 오버레이 → JSON 시드
    ekey = "enriched_rev" if is_reversed else "enriched_up"
    en = _enriched.get(str(code))
    if en and en.get(ekey):
        return str(en[ekey])
