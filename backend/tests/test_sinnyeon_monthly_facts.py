...
def test_sinnyeon_dup_months_flags_recycled_clause():
    """도입부만 바꾸고 '뒤 문구(클로즈)를 재탕'한 달을 15자+ 시그니처로 검출 — 앞40자 비교의 맹점 해소.

    [운영자 실측 2026-08-06] '일곱 번째 달에는… + 뒤 문구 복붙'을 종전 감지가 0개로 오판한 회귀 방지."""
    shared = "서로간 소통을 통해 문제를 차분히 해결해 나가시길 바랍니다"     # ≥15 한글자 재탕 클로즈
    ans = ("### ③ 월별 흐름\n\n"
           + _fake_month(1, "기축월", "월간 겁재", f"새해 첫 달에는 새로운 계획을 세우기 좋습니다. {shared}.") + "\n"
           + _fake_month(2, "경인월", "월간 식신", f"둘째 달에는 재물 흐름이 활발해집니다. 하지만 {shared}.") + "\n"  # 도입만 바꾸고 재탕
           + _fake_month(3, "신묘월", "월간 상관", "셋째 달에는 건강 관리와 충분한 휴식이 특히 중요한 시기입니다."))     # 완전 다른 내용
    assert T._sinnyeon_dup_months(ans) == [2]           # 도입만 바꾼 재탕(2월)만 검출, 3월은 불검출
    # 15자+ 공통 클로즈가 없으면 빈 리스트(깨끗한 런 불변 보장)
    clean = ("### ③ 월별 흐름\n\n"
             + _fake_month(1, "기축월", "월간 겁재", "새해에는 이러한 기회가 찾아옵니다.") + "\n"
             + _fake_month(2, "경인월", "월간 식신", "다음 달에는 전혀 다른 국면이 펼쳐집니다."))
