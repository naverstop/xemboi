...
import { ENTRY_MENU_LABEL, ENTRY_MENU_DESC, entryCost, entryFree, type EntryMenu } from "../lib/entryFee";
...
export function useEnsureEntry() {
  ...
  }, [me, openCharge, confirmEntry]);
}

/** 입장 확인 모달 — 입장료·추가질문 단가·잔액을 상세히 안내(오해 예방). 전 프리미엄 메뉴 공통. */
...
export function EntryFeeNotice({ menu }: { menu: EntryMenu }) {
  ...
  return (
    <div className={`entry-fee-notice${short ? " short" : ""}`}>
      <span className="efn-main">💎 {label} 입장료 <b>{cost.toLocaleString()}P</b> <em>· 추가 질문은 별도 차감</em>
        {/* 24h 동일입력 재사용(운영자 승인) — 현재 신년운세만 서버 dedup 적용 */}
        {menu === "sinnyeon" && <em> · 같은 정보는 24시간 내 재조회 무료</em>}
      </span>
      {me && (short ? (
