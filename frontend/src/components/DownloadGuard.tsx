 ...
 *  ② 완료 → 라벨이 tr("misc.dl_done")로 바뀜
 *  ③ 완료 후 재클릭 → tr("misc.dl_redownload_confirm") 확인 팝업(확인/취소)
 *
...
export default function DownloadGuard({
  ...
  function onClick(e: React.MouseEvent) {
    ...
    if (state === "done" &&
        !window.confirm(mode === "open" ? tr("misc.dl_reopen_confirm") : tr("misc.dl_redownload_confirm"))) return;
    lock.current = true;
    setState("busy");
    onFire?.();
    fire();
  ...
  return (
    <a className={className} href={href} onClick={onClick} aria-disabled={state === "busy"}
      ...
      {state === "busy"
        ? (busyLabel ?? (mode === "open" ? tr("misc.dl_opening") : tr("misc.dl_downloading")))
        : state === "done"
          ? (doneLabel ?? (mode === "open" ? tr("misc.dl_open_done") : tr("misc.dl_done")))
          : children}
    </a>
  );
}
