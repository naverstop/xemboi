...
const EL_HANJA: Record<El, string> = { 木: "木", 火: "火", 土: "土", 金: "金", 水: "水" };
// FLUX 베이크 오행 메달리온(양각·금테, 중앙 비움 — 글자는 웹폰트로 얹음). scripts/design/bake_wheel_medallions.py
const EL_IMG: Record<El, string> = {
  木: "/wheel/wood.jpg", 火: "/wheel/fire.jpg", 土: "/wheel/earth.jpg",
  金: "/wheel/metal.jpg", 水: "/wheel/water.jpg",
};
// 금(백금 에나멜)은 중앙이 밝아 흰 글자가 안 보임 → 짙은 글자색으로 예외 처리
const EL_TEXT_ON_MEDALLION: Record<El, string> = {
  木: "#ffffff", 火: "#ffffff", 土: "#ffffff", 金: "#6b5b2e", 水: "#ffffff",
};
        ...
        {tab === "gan"
          ? STEMS.map((s, i) => (
              <button key={s.hanja} type="button" onClick={() => setSel(i)}
                      ...
                      style={{ ...pos(i, STEMS.length, 42), "--el": EL_COLOR[s.el],
                               "--elimg": `url(${EL_IMG[s.el]})`,
                               "--eltext": EL_TEXT_ON_MEDALLION[s.el] } as React.CSSProperties}
          ...
          : BRANCHES.map((b, i) => (
              <button key={b.hanja} type="button" onClick={() => setSel(i)}
                      ...
                      style={{ ...pos(i, BRANCHES.length, 42), "--el": EL_COLOR[b.el] } as React.CSSProperties}
