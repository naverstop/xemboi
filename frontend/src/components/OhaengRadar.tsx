...
const AXES = ["목", "화", "토", "금", "수"] as const;   // 내부 데이터 키(호출측 계약 — 한국어 키)
// 표시 독음은 로케일 매핑(chart.wuxing, 한자 키) — 내부 키 → 한자 브릿지
const HJ: Record<string, string> = { 목: "木", 화: "火", 토: "土", 금: "金", 수: "水" };
type WX = Record<string, number>;

export default function OhaengRadar({
  saju, add, deficient, compLabel,
}: {
  saju: WX;                 // 사주 팔자8 오행 개수 {목,화,토,금,수}
  add?: WX;                 // 이름이 더하는 자원오행 개수(보완). 없으면 빨강만
  deficient?: string[];     // 부족 오행(축 라벨 강조)
  compLabel?: string;       // 파랑 범례 라벨(기본=로케일 '이름 보완 후' / 개명은 호출측이 전달)
}) {
  const { t: tr } = useTranslation();
  const wuxingR = tr("chart.wuxing", { returnObjects: true }) as Record<string, string>;
  ...
  return (
    <div className="ohaeng-radar">
      <svg viewBox="0 0 200 200" role="img" aria-label={tr("chart.or_aria")}>
      ...
      <div className="or-legend">
        <span className="ol-item"><i className="ol-dot saju" /> {tr("chart.or_cur")}</span>
        {add && <span className="ol-item"><i className="ol-dot comp" /> {compLabel ?? tr("chart.or_comp_default")}</span>}
      </div>
    </div>
  );
}
